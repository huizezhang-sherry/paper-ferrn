...
load(here::here("data/holes_1d_geo_polish_default_alpha.rda"))
...
```{r}
bind_rows(holes_1d_geo, holes_1d_geo_polish_default_alpha) %>% 
  explore_proj_pca(col = method)
```
...
```{r}
bind_rows(holes_1d_geo, holes_1d_geo_polish) %>% 
  explore_proj_pca(col = as.factor(alpha))
```
