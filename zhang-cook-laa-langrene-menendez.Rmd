...
```{r toy-pca-animated, fig.cap = "Six frames selected from the animated version of the previous plot. With animation, the progression of the search paths from start to finish is better identified. CRS (brown) finishes the optimisation quicker than PD (green) since there is no further movement for CRS in the sixth frame. The full video of the animation can be found in the html version of the paper.", eval=knitr::is_latex_output()}
```


```{r toy-pca-animated-interactive, out.extra="class = 'gif'", fig.cap = "Animated version of Figure 4. With animation, the progression of the search paths from start to finish can better identified and CRS (brown) finishes the optimisation quicker than PD (green).", eval=knitr::is_html_output()}
```
