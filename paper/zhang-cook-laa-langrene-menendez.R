...
## ----tour-path, out.height="20%",out.width="50%", fig.cap="An illustration for demonstrating the frames in a tour path. Each square (frame) represents the projected data with a corresponding basis. Blue frames are returned by the projection pursuit optimisation and white frames are constructed between two blue frames by geodesic interpolation."----
include_graphics(here::here("img/tour-path.png"))


## ----toy-search, fig.cap="A comparison of the searches by two optimisers: CRS (left) and SA (right) on a 2D projection problem of a six-variable dataset, \\code{boa6} using holes index. Both optimisers reach the final basis with a similar index value while it takes SA longer to find the final basis."----
...
## ----noisy-better-geo, fig.height = 4, fig.cap = "Comparison of the three optimisers in optimising $I^{nk}(n)$ index for a 1D projection problem on a five-variable dataset, \\code{boa5}. Both CRS and SA succeeds in the optimisation, PD fails to optimise this non-smooth index. Further, SA takes  much longer than CRS to finish the optimisation, it finishes off closer to the theoretical best."----
