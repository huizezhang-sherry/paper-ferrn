...
Furthermore, optimization methods can be divided in various classes (REF). In this paper, we focused on methods that rely on the objective function derivatives known as gradient methods, and those which are known as derivative free methods where optimization does not rely on the gradient or derivatives information.

Our interest is on constrained optimization as defined in the introduction section and assume that it is not possible to find a solution to the problem in the way of a closed form solution. That is, the problem consists of finding the minimum or maximum of a function \(f \in L^p\) in the constrained \(\mathbb{A}\) space. In this scenario, gradient descent or gradient ascent methods) have taken the stage for many decades and rely in the usage of the objective function derivatives.
...
\item
  A search direction
\item
  Step length --\textgreater{} line search methods ()
\item
  R available packages
\end{itemize}

Although gradient optimization methods are very popular, they rely on the existence of the objective function derivatives and the properties of the searching space. However, the assumption of having a sufficiently smooth objective function where derivatives exist, it is not adequate for all optimization problems. In addition, even in the case where derivatives exists sometimes the searching space is such that their computation is not feasible or it is computationally too expensive. Because of that, the second big class of optimization problems look at derivative free methods where the emphasis is in finding, in most cases, a near optimal solution.

\textbf{Derivative free methods}

\begin{itemize}
\tightlist
\item
  Do not rely on derivatives of the objective function
\item
