...
where \(I_d\) is the d-dimentional identity matrix. The constraint
requires the projection basis \(\mathbf{A}\) to be a orthonogal matrix
with each column vector being orthornormal.
...
tidy data principal \citep{wickham2014tidy} that states
...
observations as points in a plot, the space each iteartion take will be
...
last basis. This could be remedised by adding more layers using the
concept of gramma of graphics. A label geometry is added at the bottom
of the plot to show the number of points in each iteration and a line
geometry links the last basis in each iteration. Further, a option to
switch betwen displaying points and boxplot geometry is helpful since
point geometry can be more intuitive for the iteration with few
observations. This is avhieved via a \texttt{cutoff} parameter. Below we
...
over-emphasize the searches in the last few iterations. The second plot
design evenly presents the summarised information for each iteration
while allow for a switch to the full information for the iteration with
...
points with different plot designs. In upper plot, points from the last
three iterations span the vast majority of the plotting space leaving
the first few iterations being squeezed together whilst the lower plot
spaces each iteration evenly and presents summary information easy to
read.}
\end{figure}
...
tour paths. The upper plot shows a desirable interpolating in each
...
probablistic acceptance produces a monotonically decreasing
interpolation whilst in iteration five, six and seven, with also a
probablistic acceptance, the interpolation is now non-monotical. When
the target basis has a higher index value, iteration eight reaches this
basis by first decreases the index value. While the interpolation plot
shows different possibilities of the change in index value on the tour
path, the diagnose of the validity of each pattern and the related
...
maximises the index value and output the corresponding projection basis.
