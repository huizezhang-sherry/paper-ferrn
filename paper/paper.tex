...
construction of a visual diagnostic framework that unveil the blackbox
of optimisation. Section \ref{vis-diag} focues on motivational ideas and
data structure and Section \ref{vis-diag-plots} provides definitions of
...
projection pursuit guided tour. Coined by
...
via projecting it in lower dimensions. Let \(\mathbf{X}_{n \times p}\)
be the data matrix, an n-d projection can be seen as a linear
transformation \(T: \mathbb{R}^p \mapsto \mathbb{R}^d\) defined by
...
that maps the projection basis \(\mathbf{A}\) onto a scalar index value
\(I\), this function is commonly known as the projection pursuit index
(PPI) function, or the index function and is used to measure the
``interestingness'' of the projection. A number of index functions have
...
necessary for projection pursuit to find bases that give interesting
projections. While we leave the formal construction the optimisation
problem and existing algorithms in section \ref{tour-optim}, we want to
emphasizes here that given a random starting (current) basis, projection
pursuit repetitively searches for candidate bases nearby and outputs a
target basis once it finds one with higher index value than the current
basis. The target basis becomes the current basis in the next iteration
and the searching continues until no better basis can be found or the
...
\citet{buja2005computational} for details of the mathematical
construction of the interpolation basis Figure \ref{tour-path} shows a
sketch of the tour path consisting of the blue frames produced by
projection pursuit and the white frames, which interpolates two blue
frames. The tour method has been implemented in the \emph{tourr} package
in R, available on the Comprehensive R Archive Network at
...
using the outputted bases and interpolated bases are animated by tour;
none of the searching bases are presented in teh tour animation. The
optimisation algorithms in projection pursuit, although has been
demonstrated to find the global maximum in various literature we
referred above, focuses solely on finding the global maximum and remains
as black-box machanisms. There has been fewer tools available to explore
how each algorithm iteratively finds its maximum and compare how
different algorithms searches the parameter space. Visualisation has
been widely used for exploring and understanding data. Presenting
information in a graphical manner often allows people to see information
they would otherwise not seen. This motivates us \textbf{to develop a
visual framework to diagnose optimisation algorithms in projection
pursuit with a aim to understand and compare different existing
algorithms.} This study is meaningful because \textbf{as more complex
index functions are proposed, only an appropriate optimisation algorithm
allows us to find the projection basis that presents interesting
underlying structure in the data}.

The idea of generalised framwork for diagnostic plots is inspired by the
concept of grammar of graphic \citep{wickham2010layered}, which powers
the primary graphical system in R, ggplot2 \citep{ggplot2}. In grammar
of graphic, plots are not defined by its appearance (i.e.~boxplot,
histogram, scatter plot etc) but by ``stacked layers''. Using this
design, ggplot doesn't have to develop a gazillion of functions that
each produces a different type of plot from a different data structure.
Instead, it aesthetically maps variables (and its statistical
transformation) in a dataset to different geometric objects (points,
lines, box-and-whisker etc) and builds the plot through overlaying
different layers. There are different ways to represent the same data in
rectangular form. Certain cleaning steps are needed, before using
ggplot2, to bring the data into a tidy data format
\citep{wickham2014tidy} where 1) each observation forms a row; 2) each
variable forms a column and 3) each type of observational unit forms a
table. In a tidy format, data wrangling and visualisation are greatly
simplified.
...
For our visual diagnostics, global object is a such data construction
with key components in optimisation algorithms and is designed for
making diagnostic plots easier. In the optimisation algorithms for
projection pursuit, two key elements are the projection basis:
\(\mathbf{A}\) and the index value: \(I\). State: \(S\) labels the
observation with detailed stage of searching or interpolation.
...
search-and-interpolate round, which remains the same for within one
...
has the algorithm searched before finding one to output.
...
\(V_2 = \text{alpha}\), which is the neighbourhood parameter that
controls the size in sampling candidate bases. The data structure can
thus be shown as in Equation \ref{eq:data-structure}.
