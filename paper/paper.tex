...
{\centering \includegraphics[width=0.6\linewidth,height=0.3\textheight]{/Users/hzha400/Documents/3.PhD/research/paper-tour-vis/figures/tour_path_keynote/tour_path_keynote.001} 

}

\caption{\label{tour-path}Each square (frame) represents the projected data with a corresponding basis. Blue frames have bases found by an optimisation algorithm iteratively whilst the white frames are constructed by geodesic interpolation between two blue frames.}\label{fig:tour-path}
\end{figure}
...
\caption{\label{fig:toy-search}\label{toy-search}A comparison of \texttt{search\_better} with different parameter value on \texttt{max\_tries}. A switch of geometry from points to boxplot happens when the count in the iteration excess 15 to avoid overplotting. With \texttt{max\_tries\ =\ 500} specification, the algorithm is working much harder towards to the end.}
\end{figure}
...
\caption{\label{fig:toy-interp}\label{toy-interp} The resulting trace plot on the interpolated points has been ploted when using three different algorithms to optimise the index. The color represents the number of iteration. It can be observed that each algorithm differs in length in the optimisation and the curvature of the improvement for each algorithm also varies.}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/toy-tour-1.pdf}
...
\begin{figure}
\centering
\includegraphics{paper_files/figure-latex/geo-polish-1.pdf}
\caption{\label{fig:geo-polish}\label{geo-polish}Two-D projection on \texttt{boa6} data with holes index optimised by \texttt{search\_geodesic}. The left panel shows the final projected data before polish and the right panel shows the one after. The separation of the clusters on the y axis becomes sharper after the polish.}
\end{figure}
...
\caption{\label{fig:kol-cdf}\label{kol-cdf}One-D projection on \texttt{boa5} data with noisy index \texttt{norm\_kol} optimised by \texttt{search\_geodesic} and \texttt{search\_better}. The grey dashed line represents the index value of the theoreical best basis. \texttt{search\_geodesic} fails to optimse the noisy index while \texttt{search\_better} has made reasonable improvments to reach the index value close to the one of theoretical best basis.}
\end{figure}
