...
pacakges, where the data collection happens when a tour is running and
hence implemented in the existing \texttt{tourr} package. The
diagnostics of the algorithm via plots and other auxiliary functions
have been implemented in a new pacakge, \texttt{ferrn}.
...
Once the data object has been obtained, \texttt{ferrn} provides four
automatic diagnostic plots introduced in Section \ref{vis-diag}. The
overall structure is \ldots{}.
...
Several auxiliary functions provides a handy solution to extract certain
component in the data object. For example, \texttt{get\_best()} extracts
the final basis found by the algorithm and \texttt{get\_interp()}
extracts the interpolation observations and renumber the id for a
continuous numbering that being supplied to
\texttt{explore\_trace\_interp()}.

\hypertarget{bind-more-information}{%
\subsection{Bind more information}\label{bind-more-information}}

\texttt{bind\_*} functions allow users to incorporate further useful
information into the data object. Given a simulated dataset, the basis
that can produce the most interesting projection is known and users can
use \texttt{bind\_theoretical()} to incorporate this best possible
basis:
...
\StringTok{  }\KeywordTok{bind_theoretical}\NormalTok{(}\DataTypeTok{matrix =}\NormalTok{ theo_best, }\DataTypeTok{index =}\NormalTok{ tourr}\OperatorTok{::}\KeywordTok{holes}\NormalTok{(), }\DataTypeTok{data_raw =}\NormalTok{ data) }
\end{Highlighting}
\end{Shaded}

\texttt{bind\_random(\_matrix)} generates a large number of points on
the surface of a high dimensional sphere and attaches it to the original
data object. This is particularly useful for exploring the search space
in \texttt{explore\_space\_pca()} and \texttt{explore\_space\_tour()}
since the space of the projection basis is a high dimensional sphere and
overlaying the bases with random points on the sphere provides better
perception to view these plots.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{holes_1d_better }\OperatorTok{%>%}\StringTok{ }\KeywordTok{bind_random}\NormalTok{() }\OperatorTok{%>%}\StringTok{ }\KeywordTok{tail}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
A handful of color from Australian domestic plants has been incorporated
as the main color theme of the package, which inlcudes fern, cherry,
banksia, acacia and daisy.
