...
\CRANpkg{stats} \citep{stats} package, which provides both
...
\CRANpkg{nloptr} \citep{nloptr}. Specific solvers for simulated
annealing includes \texttt{optim(...,\ method\ \ "SANN")} and package
\CRANpkg{GenSA} \citep{gensa} that deals with more complicated objective
...
\caption[Trace plots of two optimisation procedures with color representing each iteration]{Trace plots of two optimisation procedures with color representing each iteration. The first optimiser progresses monotonically towards the opitmum while the second has experienced some reduction in some iterations.}\label{fig:toy-interp}
\end{figure}
\end{Schunk}
...
\ref{fig:toy-interp} presents the interpolation of two different tour
paths. The interpolation on the left has index value increase
monotonically while the right one has the index value decreases in
several iteration. Compare to the Figure \ref{fig:toy-search}, Figure
\ref{fig:toy-interp} provides additional information on how the
interpolation gradually reaches the target basis in each iteration:
either monotonically or experiencing a turning point, as seen on the
right of Figure \ref{fig:toy-interp}.
