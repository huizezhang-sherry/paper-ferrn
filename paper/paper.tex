...
finding the global maximum point, while the projection pursuit explores
the data with an aim to discover the potential underlying structure and
thus is interested in how the algorithm finds its maximum. This
motivates us \textbf{to develop a visual diagnostic system for exploring
the optimisation searching path}.
...
Tidy data, as introduced in \citep{wickham2014tidy}, has three criteria:
1) each observation forms a row, 2) each variable forms a column and 3)
each type of observational unit forms a table. The adoption of tidy data
makes it easier for data wrangling and visualisation. For our visual
diagnostics, global object is a such data construction with key
components in optimisation algorithms and is designed for making
diagnostic plots easier. In the optimisation algorithms for projection
pursuit, two key elements are the projection basis: \(\mathbf{A}\) and
the index value: \(I\). State: \(S\) labels the observation with
detailed stage of searching or interpolation.
