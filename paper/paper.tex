...
found in \citet{buja2005computational}. {[}Maybe add the sketch for
illustrating tour path.{]} Guided tour, along with other types of tour,
has been implemented in the \emph{tourr} package in R, available on the
Comprehensive R Archive Network at
\url{https://cran.r-project.org/web/packages/tourr/}
\citep{wickham2011tourrpackage}.

\begin{figure}
\includegraphics[width=1\linewidth,height=0.6\textheight]{/Users/hzha400/Documents/3.PhD/research/paper-tour-vis/figures/tour_path_increasing} \caption{\label{tour-path}An illustration of the tour path}\label{fig:tour-path}
\end{figure}
...
There are three existing methods for optimising PPI function and we
review them below.

\citet{posse1995projection} proposed a stochastic direct search method,
a random search algorithm. Given a current basis
\(\mathbf{A}_{\text{cur}}\), a candidate basis \(\mathbf{A}_{t}\) is
sampled in the neighbourhood defined by the radius of the p-dimensional
sphere, \(\alpha\) of the current basis \(\mathbf{A}_{t}\) by
\(\mathbf{A}_{t} = \mathbf{A}_{\text{cur}} + \alpha \mathbf{A}_{rand}\).
If the candidate basis has a higher index value than the current basis,
it is outputted as the target basis \(\mathbf{A}_{t}\) along with other
...
and when \(c > 30\), the neighbourhood parameter \(\alpha\) in the next
...
\(T(t)\). Given an initial \(T_0\), the temperature at iteration \(t\)
is defined as \(T(t) = \frac{T_0}{log(t + 1)}\). The simulated annealing
...
\(P= \min\left\{\exp\left[\frac{I_{t+1} - I_{t}}{T(t)}\right],1\right\}\)
where \(I_{t}\) and \(I_{t+1}\) are the index value of the current and
...
and the \emph{value of the objective function: \(I\)}, which is the
projection basis and index value repsectively in projection pursuit.
\emph{State: \(S\)} is another variable of great importance since it
labels \(\mathbf{A}\) and \(I\) by the stage they are in the whole
search-and-interpolate process.

\emph{Iterators} indexes the data collected and prescribes the the
natural ordering of each observation in different levels. \emph{\(t\)}
is a unique identifier that has an increment of one for each observation
and \emph{\(j\)} is the counter for each search-and-interpolate round.
{[}add a bit more for \emph{\(l\)} after decide whether to combine \(l\)
and \(k\). {]}

There are other parameters that are of our interest and we denote them
as \emph{\(V_{p}\)}. In projection pursuit, this includes
\(V_1 = \text{method}\), denoting the name of the searching method used
and \(V_2 = \text{alpha}\), which is a neighbourhood parameter that
controls the size in sampling candidate bases. A sketch of the global
object for projection pursuit guided tour is presented in Figure
...
\item
  ...
  \item
    y: the statistical transformed index value:
    \(Q_q(I_{T} \cdot \mathds{1} (S = \text{search}))\) is mapped to the
    y-axis where \(q\) takes one of \(0, 25, 50, 75, 100\) and
    \(Q_q(x)\) finds the qth-quantile for the vector \(x\).
...
\hypertarget{implementation-ferrn-package}{%
\section{Implementation: Ferrn
package}\label{implementation-ferrn-package}}
