...
\texttt{search\_better} may get stuck at a local maximum. In the case
...
optimiser: \texttt{search\_better}. The first two interpolation are
identical while in the third interpolation, a better basis has been
found than the target one. Rather than starting the next iteration from
the ``target basis'' on Time 62, as shown in the left panel, the
