...
\abstract{%
Guided tour \citep{cook1995grand} searches for interesting
low-dimensional views of high-dimensional data via optimising a
projection pursuit index function. The first paper of projection pursuit
by \citet{friedman1974projection} stated that ``the technique used for
...
evaluating the performance of the optimisers. In this paper, we propose
a data collection object in the optimisation of projection pursuit
guided tour and introduce visual diagnostics for examining the
performance of the optimisation procedure. These diagnostics and this
workflow can be applied to a broad class of optimisers to assess their
performance. An R package, \CRANpkg{ferrn} \citep{ferrn}, has been
...
be used to examine the performance of different optimisers and guide
improvements to the existing algorithms. Finally, Section
...
Random search methods \citep{zabinsky2013stochastic, Romeijn2009} have a
random sampling component as part of their algorithm and have been shown
to have the ability to optimise non-convex and non-smooth functions. The
