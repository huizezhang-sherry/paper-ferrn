...
\caption[Examing the index values as the optimisation progress for two optimisers using trace plot]{Examing the index values as the optimisation progress for two optimisers using trace plot. The lines indicate the interpolation and the dots indicate a new target is generated.}(\#fig:toy-interp)
\end{figure}
\end{Schunk}
...
\caption{Six selected frames from the animated PCA plots in the previous figure. With animation, the start and end position of each optimisation is easier to identify.  A full video of this aimation can be found at \url{https://vimeo.com/user132007430/review/504242845/b73f37175a}}(\#fig:toy-pca-animated)
\end{figure}
\end{Schunk}
