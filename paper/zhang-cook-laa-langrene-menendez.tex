...
some structures of the data, e.g., bi-modality, clustering and outliers,
...
cases, a near optimal solution. Below will introduce three
derivative-free methods, two of which are random search methods:
creeping random search and simulated annealing, and the other one is
pseudo derivative search.

Random search mehtods \citep{zabinsky2013stochastic} contain a random
sampling component in its algorithm and has shown its ability to
optimise non-convex and non-smooth functions. The initial random search
algorithm, pure random search \citep{Brooks1958-zk}, draws candidate
points from the entire space without using any information of the
current position and updates the current position when an improvement on
the objective function is made. As the dimension of the space becomes
larger, sampling from the entire space would indicate a long time for
convergence to occur despite a guaranteed global convergence
\citep{spall2005introduction}. Various algorithms have then been
developed to improve pure random search by either concentrating on a
narrower sampling space or using a different updating machanism.
Creeping random search \citep{White1971-ci} is such a variation where a
candidate point is generated within a neighbourhood of the current
point. This gives creeping random search an advantage of being fast to
compute, however, global convergence is not longer guaranteed. On the
other hand, simulated annealing
\citep{kirkpatrick1983optimization, bertsimas1993simulated}, takes a
different updating machanism from the pure random search. Rather than
only updating the current point when an improvement is made, simulated
annealing uses a Metropolis acceptance criterion, where nonimproving
candidates can still have a chance to be accepted. The convergence of
simulated annealing algorithms has been widely researched
...
is repeated until the desire convergence is reached. In derivative-free
methods, local information of the objective function are used to
determine the search direction. The choice of step size also needs
considerations, as inadequate step sizes might prevent the optimisation
method to converge to an optimum. An ideal step size can be chosen via
