...
implement a data collection object in the optimisation of projection
pursuit guided tour and introduce visual diagnostics based on the data
object collected. These diagnostics and this workflow can be applied to
a broad class of optimisers, to assess their performance. An R package,
\CRANpkg{ferrn} \citep{ferrn}, has been created to implement the
diagnostics.
...
defined via an objective function. Broadly speaking, optimization can be
...
projection, represented by the projection matrix, that gives the most
interesting low-dimensional view. A view is interesting if it can show
some structures of the data other than a normal distribution, e.g.,
bi-modality, clustering and outliers.
...
and optimization methods presented in this paper can therefore easily be
...
guide improvements to the existing algorithms. Finally, Section
\ref{implementation} describes the R package: \CRANpkg{ferrn}
\citep{ferrn}, that implements the visual diagnostics.
...
A large class of methods uses the gradient information of the objective
function, with the most notable one being the gradient ascent (descent)
method. Although gradient optimization methods are popular, they rely on
the availability of the objective function derivatives and on the
limited complexity of the constraints. As will be shown in the next
section, the independent variables in our optimisation problem are the
entries of a projection matrix and the computational time required to
perform differentiation on a matrix could impede the render of tour
...
cases, a near optimal solution. Here we consider three derivative-free
methods, two of which are random search methods: creeping random search
and simulated annealing, and the other one is pseudo derivative search.

Random search methods \citep{zabinsky2013stochastic} have a random
sampling component as part of their algorithm and have been shown to
have the ability to optimise non-convex and non-smooth functions. The
initial random search algorithm, pure random search
...
and the global optimum can be attained under mild conditions.
...
to be normal, as \(n\) and \(p\) approach infinity under certain
conditions. Index functions proposed in the litearture include early
indices that can be categorised by measuring the \(L^2\) distance
between the projection and a normal distribution: Legendre index
...
{\centering \includegraphics[width=0.5\linewidth,height=0.2\textheight]{/Users/ursulalaa/papers/paper-tour-vis/img/tour-path} 
...
searches it has conducted, i.e.~we to summarise how the index is
increasing over iterations and how many tries need to be sampled at each
iteration. This is achieved using the search plot function: a boxplot
shows the distribution of index values for each try, where the selected
basis (corresponding to the highest index value) is always shown as a
point. In case of few tries at a given iteration showing the data points
directly may be preferred over the boxplot, this is controlled via the
...
highlighted in a larger size; and 3) the colour of the last iteration is
in a grey scale to indicate no better basis found in this iteration.
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-search-1} 

}

\caption[A comparison of the searches by two optimisers]{A comparison of the searches by two optimisers: CRS (left) and SA (right) on a 2D projection problem of a six-variable dataset, \code{boa6} using holes index. Both optimisers reach the final basis with a similar index value while it takes SA longer to find the final basis.}\label{fig:toy-search}
\end{figure}
\end{Schunk}
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-interp-1} 
...
is shown in the tour animation. Trace plots are created by plotting the
index value against time. Figure \ref{fig:toy-interp} presents the trace
plot of the same optimisations as Figure \ref{fig:toy-search} and one
can observe that the trace is smooth in both cases. It may seem bizarre
at the first sight that the interpolation sometimes passes bases with
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-pca-1} 
...
This happens because the projection is invariant to orientation of the
basis and so is the index value. However, this creates difficulties for
comparing the optimisers since the end bases will be symmetric with
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-pca-animated-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-tour-1} 
...
orthogonality. Two paths from CRS and PD are plotted on top of the torus
and coloured in green and brown respectively to match the previous
plots. The final basis found by PD and CRS are shown in a larger shape
and printed below respectively:
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/toy-torus-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/interruption-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/polish-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/noisy-better-geo-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/kol-result-1} 
...
{\centering \includegraphics[width=1\linewidth]{/Users/ursulalaa/papers/paper-tour-vis/figs/flip-sign-1} 
...
data collection object is implemented in \CRANpkg{tourr} \citep{tourr},
while the visual diagnostics of the optimisers is implemented in
\CRANpkg{ferrn} \citep{ferrn}. Below list functions in the
\CRANpkg{ferrn} \citep{ferrn} package:
...
progression of the optimiser and the coverage in search space. The
...
existing \CRANpkg{tourr} \citep{tourr} package; while the manipulation
and visualisation of the data object are implemented in the new
\pkg{ferrn} package. Equipped with handy tools to diagnose the
