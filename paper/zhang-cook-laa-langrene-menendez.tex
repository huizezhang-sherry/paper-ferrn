...
\caption[1D projection on the 5-variable dataset \code{boa5}5) with two optimisers]{1D projection on the 5-variable dataset \code{boa5}5) with two optimisers: Simulated Annealing (SA) and Pseudo Derivative (PD). All the bases in pseudo derivative has been flipped positive so that the two paths finish close to each other and to the theoretical best.}\label{fig:toy-pca}
\end{figure}
\end{Schunk}
