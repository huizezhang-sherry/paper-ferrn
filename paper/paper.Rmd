...
```{r toy-search, fig.asp=0.5,  fig.cap="\\label{toy-search}A comparison of \`search_better\` with different parameter value on \`max_tries\`. A switch of geometry from points to boxplot happens when the count in the iteration excess 15 to avoid overplotting. With \`max_tries = 500\` specification, the algorithm is working much harder towards to the end."}
...
Animating the plots introduced above is useful, especially in the case of PCA polot since it shows the bases found by the optimiser in its natural order. In Figure \ref{toy-pca-animated}, ...
