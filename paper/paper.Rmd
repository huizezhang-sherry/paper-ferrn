...
```{r tour-path, out.height="30%", out.width="60%", fig.align='center', fig.cap="\\label{tour-path}Each square (frame) represents the projected data with a corresponding basis. Blue frames have bases found by an optimisation algorithm iteratively whilst the white frames are constructed by geodesic interpolation between two blue frames."}
...
```{r toy-interp, fig.height = 3, fig.cap = "\\label{toy-interp} The resulting trace plot on the interpolated points has been ploted when using three different algorithms to optimise the index. The color represents the number of iteration. It can be observed that each algorithm differs in length in the optimisation and the curvature of the improvement for each algorithm also varies."}
...
```{r toy-pca, fig.height = 3, fig.cap = "\\label{toy-pca}PCA plot of search geodesic colouring by info allows for better understanding of each stage in the geodesic search"}
...
```{r fig.height = 3, fig.cap = "\\label{toy-pca-aniamted}A selected number of frames from the animated PCA plot. With animation, it is easier to track the progrssion from the start to finish in each algorithm."}
...
Animating the plots introduced above is useful, especially in the case of PCA polot since it shows the bases found by the optimiser in its natural order. In Figure \ref{toy-pca-animated}, There should actually be more writing here to and also the this examples may need to be changed. 
...
```{r geo-polish, fig.height= 2.5, fig.cap = "\\label{geo-polish}Two-D projection on \`boa6\` data with holes index optimised by \`search_geodesic\`. The left panel shows the final projected data before polish and the right panel shows the one after. The separation of the clusters on the y axis becomes sharper after the polish."}
# set.seed(123456)
# render(
#   boa6,
...
```{r kol-cdf,fig.asp=0.5,fig.cap = "\\label{kol-cdf}One-D projection on \`boa5\` data with noisy index \`norm_kol\` optimised by \`search_geodesic\` and \`search_better\`. The grey dashed line represents the index value of the theoreical best basis. \`search_geodesic\` fails to optimse the noisy index while \`search_better\` has made reasonable improvments to reach the index value close to the one of theoretical best basis."}
# set.seed(123456)
# kol_1d_geo <- 
...
```{r pca-better-random-default, fig.height = 6, fig.cap = "\\label{pca-better-random-default}One-D projection of \`norm_kol\` index on \`boa6\` data optimised by \`search_better_random\` with the same seeds. \`search_better_random\` has a probablistic acceptance implementation that would also a basis with lower index value. This design allows the optimiser to jump out of the local maximum and hence more incidents find the true global maximum. "}
set.seed(123)
seed <- sample.int(10000, 20)
...
```{r pca-better-random-tuned, fig.height= 6, fig.cap = "\\label{pca-better-random-tuned}One-D projection of \`norm_kol\` index on \`boa6\` data optimised by \`search_better_random\` with the same seeds. Further tuning of the parameter has been taken place to adjust the neighbourhood parameter \`alpha\` from 0.5 to 0.7. This time, all the 20 incidents have found the global maximum."}
set.seed(123)
seed <- sample.int(10000, 20)
