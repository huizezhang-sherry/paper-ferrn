...
```{r seed-table, eval = FALSE}
best <- get_best(polish_tidy, group = seed)

all_summary <- best$basis %>% 
  purrr::flatten_dbl() %>% matrix(ncol = 6, byrow = TRUE) %>% 
  as_tibble() %>% 
  rename("V7" = "V3", "V8" = "V4", "V9" = "V5", "V10" = "V6") %>% 
  mutate(IndexVal = best$index_val, seed = as.factor(seed)) %>% 
...
all_summary %>% knitr::kable(digits = 3, caption = "this is the caption", format='latex', booktabs=TRUE, escape=FALSE)
...
pca$aug %>% filter(method != "randomly_generated"& info != "theoretical") %>% 
  ...
  geom_point(aes(col = VarFound), alpha = 0.5) + 
  ...
  geom_point(data = pca$aug %>% get_best(group = seed), size = 7) +
...
all_tidy %>% mutate(seed = as.factor(seed)) %>% 
  left_join(all_summary, by = "seed") %>% 
  explore_trace_interp(group = seed, col = VarFound) + 
  facet_wrap(vars(fct_reorder(seed, IndexVal)))  + 
...
dt <- bind_rows(all_tidy %>% filter(seed == 1842) %>% mutate(algo = "better V2"),
          all_tidy %>% filter(seed == 9982) %>% mutate(algo = "better V7"),
          kol_1d_2var_better_random %>% mutate(algo = "better random"))
...
p2 <- pca$aug %>% filter(method != "randomly_generated" & info != "theoretical") %>% 
  ...
  geom_point(data = pca$aug %>% get_start(),  size = 3) +
