...
Section \ref{tour} introduces the optimisation problem in a specific context: projection pursuit guided tour and existing derivative free algorithms for solving it. 
Section \ref{vis-diag} and \ref{vis-diag-plots} together present the new construction of a visual diagnostic framework that unveils the black-box of optimisation. Section \ref{vis-diag} focuses on motivational ideas and data structure and Section \ref{vis-diag-plots} gives definitions of different diagnostic plots using grammar of graphic and examples. 
...
In modern optimisation problems, gradient information can be hard to evaluate or sometimes even impossible and  Derivative-Free Optimisation (DFO) methods can be useful to approach these problems.
A traditional and still reliable in solving complicated practical problems, is *line search method* [@fletcher2013practical]. In the context of a simple one-dimensional problem of finding the $x$ that minimises $f(x)$, line search achieves the goal via an iterative algorithm in the form of Equation \ref{eq:line-search}.
