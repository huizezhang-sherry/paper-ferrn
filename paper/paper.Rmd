...
Projection pursuit is usually used in conjunction with a tour method called *guided tour*. Tour explores the multivariate data *interactively* via playing a series of projections, that form a *tour path* and guided tour uses the path that is geodesically the shortest. Details of the mathematical construction of a tour path can be found in @buja2005computational.  [Maybe add the sketch for illustrating tour path.] Guided tour, along with other types of tour, has been implemented in the *tourr* package in R, available on the Comprehensive R Archive Network at [https://cran.r-project.org/web/packages/tourr/](https://cran.r-project.org/web/packages/tourr/) [@wickham2011tourrpackage]. 
...
@posse1995projection proposed a stochastic direct search method, a random search algorithm. Given a current basis $\mathbf{A}_{\text{cur}}$, a candidate basis $\mathbf{A}_{t}$ is sampled in the neighbourhood defined by the radius of the p-dimensional sphere, $\alpha$ of the current basis $\mathbf{A}_{t}$ by $\mathbf{A}_{t} = \mathbf{A}_{\text{cur}} + \alpha \mathbf{A}_{rand}$. If the candidate basis has a higher index value than the current basis, it is outputted as the target basis $\mathbf{A}_{t}$ along with other metadata and the current iteration stops. If no basis is found to have higher index value after the maximum number of tries $l_{\max}$, the algorithm stops with nothing outputted. $c$ is the halfing parameter and when $c > 30$, the neighbourhood parameter $\alpha$ in the next iteration will be reduced by half. The algorithm for a random search is summarised in Algorithm \ref{random-search}.
...
\SetAlgoLined
  ...
  \SetKwInOut{Output}{output}
    ...
    \Output{The target basis: $\mathbf{A}_{t+1}$}
  initialisation\;
  Set $l = 1$\;
  \While{$l < l_{\max}$}{
...
Simulated annealing [@bertsimas1993simulated, @kirkpatrick1983optimization] is a non-derivative procedure based on a non-increasing cooling scheme $T(t)$. Given an initial $T_0$, the temperature at iteration $t$ is defined as $T(t) = \frac{T_0}{log(t + 1)}$. The simulated annealing algorithm works as follows. Given a neighbourhood parameter $\alpha$ and a randomly generated orthonormal basis $A_{rand}$, a candidate basis is constructed as $\mathbf{A}_{t+1} = (1 - \alpha)\mathbf{A}_{t} + \alpha \mathbf{A}_{rand}$. If the index value of the candidate basis is larger than the one of the current basis, the candidate basis becomes the target basis. If it is smaller, the candidate is accepted with probability $P= \min\left\{\exp\left[\frac{I_{t+1} - I_{t}}{T(t)}\right],1\right\}$ where $I_{t}$ and $I_{t+1}$ are the index value of the current and candidate basis respectively. The algorithm can be summarised as in Algorithm \ref{simulated_annealing}.
...
Ggplot requires a data frame that contains all the variables to plot and a *global object* is constructed as the data frame supplied to the visual diagnostic plots to better suit the characters of iterative optimisation algorithms. Given an optimisation algorithm, two primary variables of interest are the *decision variable: $\mathbf{A}$* and the *value of the objective function: $I$*, which is the projection basis and index value repsectively in projection pursuit. *State: $S$* is another variable of great importance since it labels $\mathbf{A}$ and $I$ by the stage they are in the whole search-and-interpolate process.

*Iterators* indexes the data collected and prescribes the the natural ordering of each observation in different levels. *$t$* is a unique identifier that has an increment of one for each observation and *$j$* is the counter for each search-and-interpolate round. [add a bit more for *$l$* after decide whether to combine $l$ and $k$. ]
...
There are other parameters that are of our interest and we denote them as *$V_{p}$*. In projection pursuit, this includes $V_1 = \text{method}$, denoting the name of the searching method used and $V_2 = \text{alpha}$, which is a neighbourhood parameter that controls the size in sampling candidate bases. A sketch of the global object for projection pursuit guided tour is presented in Figure \ref{fig:glb-obj}. The full data structure can thus be shown as in Equation \ref{eq:data-structure}.

\begin{equation}
\left[
\begin{array}{c|ccc|ccc|ccc}
t & \mathbf{A} & I & S & j &  l  & k & V_{1}& \cdots & V_{p}\\
\hline
1 & \mathbf{A}_1 & I_1 & S_1 & 1 & 1 & \ast & V_{1, 1} & \cdots & V_{p, 1}\\
\hline
2 & \mathbf{A}_2 & I_2 & S_2 & 2 & 1 & \ast & V_{1, 2} & \cdots & V_{p, 2}\\
3 & \mathbf{A}_3 & I_3 & S_3 & 2 & 2 & \ast & V_{1, 3} & \cdots & V_{p, 3}\\
\vdots & \vdots &\vdots &\vdots & \vdots &\vdots & \vdots &\vdots &\cdots &\vdots\\
\vdots & \vdots & \vdots &\vdots & 2 & l_1 & \ast & \vdots &  \cdots & \vdots\\
\hline
\vdots &\vdots & \vdots &\vdots & 2 & \ast & 1& \vdots &  \cdots & \vdots\\
\vdots &\vdots &\vdots &\vdots & 2 & \ast & 2& \vdots& \cdots & \vdots\\
\vdots &\vdots &\vdots &\vdots &\vdots  &\vdots & \vdots &\vdots &\cdots &\vdots \\
\vdots &\vdots &\vdots &\vdots & 2 & \ast & k_1&\vdots & \cdots & \vdots\\
\hline
\vdots &\vdots &\vdots &\vdots &\vdots  &\vdots & \vdots &\vdots &\cdots &\vdots \\
T & \mathbf{A}_T & I_T &S_T  & J &  l_J & \ast & V_{1, T}& \cdots & V_{p, T}\\
\end{array}
\right]
...
- Layer 1: boxplot geom
  ...
  - y: the statistical transformed index value: $Q_q(I_{T} \cdot \mathds{1} (S = \text{search}))$ is mapped to the y-axis where $q$ takes one of $0, 25, 50, 75, 100$ and $Q_q(x)$ finds the qth-quantile for the vector $x$. $\mathds{1}(.)$ is the identity operator.
