...
preamble: >
  \usepackage{amssymb, amsmath, mathtools, dsfont, bbm, array}
...
```{r fig.cap = "THis is the caption", fig.height=4, fig.align="center"}
...
holes_2d_better %>% mutate(max_tries = 25) %>% 
  ...
  explore_trace_interp(col = max_tries) + 
  theme(legend.position = "none")
```
...
Once the data object has been obtained, the package, `ferrn`, provides automatic diagnostic plots as described in Section \ref{vis-diag}. The structure of package functionality has been outlined in Table \ref{table:ferrn-glossary}. The four main functions plots, `explore_trace_search()`, `explore_trace_interp()`, `explore_space_pca()`, and `explore_space_tour()`, produce the diagnostic plots upon a simple supply of the data objects. Auxiliary functions, `get_*()` and `bind_*()`, provide handy solutions to extract certain components in the data object. They modulate the extraction and binding procedures in the main functions while being convenient to use on their own. 
...
## Extract pieces from the data object 

 `get_*()` extract and, in some cases, manipulate certain components from the existing data object, for example,  `get_best()` extracts the best basis found by the algorithm and `get_interp()` extracts the observations in the interpolation and prepare them ready to be supplied to `explore_trace_interp()`. 

## Bind external information

`bind_*()` incorporate additional information that is outside the tour optimisation. Given a simulated dataset, the most interesting projection is known ahead of time, so does its basis.  `bind_theoretical()` can be utilised to incorporate this best possible basis to the existing data object with the supply of the index function and original data for producing the index value. This binding is used in the space plots to compare the position of the theoretical best basis to the ones being searched by the algorithm. The following chunk presents its usage.

```{r bind-theoretical-demo, eval = FALSE, echo = TRUE}
theo_best <- matrix(c(0, 1, 0, 0, 0), nrow = 5)
holes_1d_better %>% 
  bind_theoretical(matrix = theo_best, index = tourr::holes(), raw_data = data) 
```

`bind_random()` and `bind_random_matrix()` generates a large number of points on the surface of a high dimensional sphere and attaches it to the existing data object. This is also useful in the space plots since it preserves the spherical structure of the spaces of the projection bases, in the computation of PCA for `explore_space_pca()` and animation of `explore_space_tour()`. 

## Botanical color palette and reorder factor levels 

A collection of color palettes from Australian native plants has been created to match the theme of the package Quantitative palettes, daisy, banksia and cherry, are recommended for comparing between algorithms, while sequantial palettes, fern and acacia are better for trace plots. These color palettes can be accessed via 

- `scale_color_botanical(palette = "...")` for ggplot constructions, and 

- `explore_space_tour(..., color = botantical_palettes$...)` for a full space exploration

When colored by `info`, users may want to map a particular color to a certain level to emphasize certain elements in the algorithm, `relevel_*()` allows the user to manipulate the level in the `info` column in the data object with the default being the chronological order that each level occurs in the optimisation.
