...
```{r toy-search, fig.height=5.5, fig.cap="\\label{toy-search}A comparison of plotting the same search points with different plot designs. In upper plot, points from the last three iterations span the vast majority of the plotting space leaving the first few iterations being squeezed together whilst the lower plot spaces each iteration evenly and presents summary information easy to read."}
...
p2 <- dt %>% ggplot() + 
  geom_point(aes(x = id, y = index_val, col = as.factor(tries))) + 
...
Figure \ref{toy-interp} shows the plot of interpolating points and it shows the algorithm starts from around 0.8 and with interpolation, gradually reaches over 0.95. 

```{r toy-interp, fig.height = 3, fig.cap = "\\label{toy-interp} This is a toy example of plotting interpolated points"}
