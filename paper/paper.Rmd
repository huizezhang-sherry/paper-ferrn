...
Once the data object has been obtained, `ferrn` provides four automatic diagnostic plots introduced in Section \ref{vis-diag}. The overall structure is ....

## Extract pieces from the data object 

Several auxiliary functions provides a handy solution to extract certain component in the data object. For example, `get_best()` extracts the final basis found by the algorithm and `get_interp()` extracts the interpolation observations and renumber the id for a continuous numbering that being supplied to `explore_trace_interp()`. 


## Bind more information

`bind_*` functions allow users to incorporate further useful information into the data object. Given a simulated dataset, the basis that can produce the most interesting projection is known and users can use `bind_theoretical()` to incorporate this best possible basis:

```{r eval = FALSE, echo = TRUE}
theo_best <- matrix(c(0, 1, 0, 0, 0), nrow = 5)
holes_1d_better %>% 
  bind_theoretical(matrix = theo_best, index = tourr::holes(), data_raw = data) 
```

`bind_random(_matrix)` generates a large number of points on the surface of a high dimensional sphere and attaches it to the original data object. This is particularly useful for exploring the search space in `explore_space_pca()` and `explore_space_tour()` since the space of the projection basis is a high dimensional sphere and overlaying the bases with random points on the sphere provides better perception to view these plots. 

```{r eval = FALSE, echo = TRUE}
holes_1d_better %>% bind_random() %>% tail()
```

## Botanical color palette and reorder factor levels 

A handful of color from Australian domestic plants has been incorporated as the main color theme of the package, which inlcudes fern, cherry, banksia, acacia and daisy. 



\begin{table}
\centering
\begin{tabular}{|wl{5cm} wl{5cm}|}
\hline
...
get\_basis\_matrix() & \\
& \\
relevel\_geo() & botanical\_palettes() \\
relevel\_better() & botanical\_pal() \\
& scale\_color\_botanical()\\
\hline
\end{tabular}
\caption{A glossary of functions available in ferrn package. The main functions are four automatic plotting functions and auxiliary functions provides easier extraction (get\_*) and binding (bind\_*) of the data object.}
