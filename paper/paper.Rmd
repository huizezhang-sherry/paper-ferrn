...
```{r tour-path, out.height="30%", out.width="60%", fig.align='center', fig.cap="\\label{tour-path}An illustration of the tour path"}
...
bind_rows(holes_2d_better_max_tries %>% mutate(max_tries = 500), 
  ...
  facet_wrap(vars(max_tries), ncol = 2)
```
...
```{r kol-cdf,fig.asp=0.5, fig.height = 3,  fig.cap = "\\label{kol-cdf}Trace plot of search_geodesic and search_better on 1D projection problem for a noisy index: norm_kol. Search_geodesic has varely made progression while search_better has successfully found the informative "}
# set.seed(123456)
# kol_1d_geo <- 
