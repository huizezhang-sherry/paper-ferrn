...
Checking the final projection before and after applying the polish as in Figure \ref{geo-polish}, we can see that ...

```{r geo-polish, fig.height= 2.5, fig.cap = "\\label{geo-polish}On the left is the final projection from search geodesic while on the right is the polished result. Polishing helps to sharpen the edges of the clusters"}
# set.seed(123456)
# render(
#   boa6,
...
```{r kol-pca, fig.cap = "\\label{kol-pca}THis is the caption"}
...
p2 <- pca$aug %>% filter(method != "randomly_generated" & info != "theoretical") %>% 
  ...
  geom_point(aes(col = algo), alpha = 0.1) + 
  ...
  geom_point(data = pca$aug %>% get_start(),  size = 5) +
