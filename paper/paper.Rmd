...
dt <- bind_rows(holes_2d_better %>% mutate(algo = "not satisfactory"),
                holes_2d_better_max_tries %>% mutate(algo = "satisfactory")) %>%
 bind_rows(holes_2d_theory %>% mutate(algo = "satisfactory")) %>%
  bind_rows(holes_2d_theory %>% mutate(algo = "not satisfactory")) %>%
  mutate(algo = fct_relevel(algo, c("satisfactory", "not satisfactory")))%>%
  compute_pca() %>%
  `$`(aug)


dt %>% explore_proj_pca(facet = algo, size = 7, alpha = 0.5)
#dt %>% explore_proj_pca(facet = algo, axis = 2)
```
...
```{r animated-pca, fig.height = 8, fig.cap = "\\label{animated-pca} 16 frames from animated PCA plot ." }
rl <- lapply(list.files(here::here("figures/animated_pca/"),full.names = TRUE)[1:16], png::readPNG)
...
holes_2d_better %>% mutate(max_tries = 25) %>% 
  dplyr::bind_rows(holes_2d_better_max_tries %>% mutate(max_tries = 500)) %>% 
  explore_trace_interp(col = max_tries, facet = max_tries) + 
  theme(legend.position = "none")
```
...
# p1 <- bind_rows(kol_1d_geo, kol_1d_geodesic_polish) %>% explore_trace_interp(col = method)  + ggtitle("search_geodesic")
# 
# p2 <- bind_rows(kol_1d_better, kol_1d_better_polish) %>% explore_trace_interp(col = method) + ggtitle("search_better")
# 
# (p1 | p2 ) & theme(legend.position = "bottom") & ylim(0, 0.23)


p1 <- kol_1d_geo %>% explore_trace_interp(col = method)  + ggtitle("search_geodesic")

p2 <- kol_1d_better %>% explore_trace_interp(col = method) + ggtitle("search_better")
...
colour <- ferrn::botanical_pal()(3)
p5 <- kol_1d_2var_better %>% explore_trace_interp(col = 0) + 
  scale_colour_manual(values = colour[2]) + ggtitle("search_better, seed 123456")
p6 <- kol_1d_2var_better_2 %>% explore_trace_interp(col = 0) + 
...
<!--   mutate(algorithm = c(rep("better 1", nrow(interp1)),  -->
<!--                        rep("better 2", nrow(interp2)),  -->
<!--                        rep("better random", nrow(interp3))), -->
<!--          algorithm = fct_relevel(algorithm, c("better 1", "better 2", "better random")))  -->

<!-- dt %>%  -->
<!--   explore_proj_pca(col = algorithm, alpha = 0.5) +  -->
<!--   scale_colour_manual(values = color(3))  -->
<!-- ``` -->
...
1. how does the printed data object looks like?
