...
The first question can be answered using a static plot with x-axis showing the progression of the optimisation and y-axis showing the value of the objective function. The second question can be addressed via visualising the rotating high dimensional space or its projection on the reduced 2D space. Thus animated visualisation is needed to preceive the optimisation path in the searching space.
...
- Layer 1: boxplot geom
  ...
  - y: the index value after statistical transformation $y = \{Q_q(I_{jlk} \cdot \mathds{1} (j = x, k = \ast))\}$ is mapped to the y-axis where $q$ takes one of $0, 25, 50, 75, 100$ and $Q_q(x)$ finds the qth-quantile for the vector $x$. $\mathds{1}(.)$ is the identity operator.
...
Sometimes, it is more interesting to explore the index value on the interpolation path and the plot definition is slightly varied.
