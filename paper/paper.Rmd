...
The projection bases $\mathbf{A}_{p \times d}$ live on the surface of a $p \times d$ dimension sphere, where this dimension can easily go from 5 to 10. This makes viewing the original space hard. An easier alternative is to view the reduced space via some dimensional reduction methods i.e. Principal component analysis. The visualisation can thus be defined as
...
Figure \ref{toy-pca} shows the first two principal components of two searching paths. A Hollow sphere with the same dimension is overlaid to emphasize the imposition of orthonormal constraint. Starting from the same position, two searching methods takes different paths to get to the two final bases with the same data projection $Y$ (the two correspond to a rotation of the data in the projection). The distinction between the two is that `search_better()` also involve some random evaluation in the sphere due to its sampling process while `search_geodesic()` doesn't. This stochastic rather than deterministic approach can be useful in more complex scenario.
...
An animated plot can be useful because it allows us to track the progression of the optimsiation, especially in the case of inspecting the search space.
...
# rl <- lapply(list.files(here::here("figures/animated_pca_1d/"),full.names = TRUE)[62:78], png::readPNG)
...
As mentioned previously, the orthonormal constraint regulates the projection basis to be living on the surface of a unit sphere. The visualisation of the projection basis on its original $p \times d$ dimension can be done through an animated rotation of the $p \times d$ diemnsion sphere with the searching path overlaid. The high dimensional geometry can be simulated using the package `geozoo` and the animated rotation can be done via a regular grand tour or a slice tour [@laa2020slice].

Compared with the PCA plot, this animated rotation display requires skills of the viewer to preceive the high dimensional space in rotation and what the viewers gain is a much holistic view of the path in the space in the sense that PCA does a further projection on the projection basis along with the hollow sphere, which gives the view that maixmises the variance. While the animated rotation display gives the view from different anlges and the PCA plot consitutes one frame from the animated rotaiton display.
