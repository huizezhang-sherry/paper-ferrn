...
preamble: >
  \usepackage{amssymb, amsmath, mathtools, bbm}
...
When aggregated with information of index value and other interesting parameter, it is easier to write the basis as a column vector as in Equation \ref{eq:vectorA} and the full data structure is presneted in Equation \ref{eq:data-structure}.


All the basis can also be denoted usign a single subscript by their natural ordering as $\mathbf{A}_0, \mathbf{A}_1, \cdots, \mathbf{A}_n$. 
...
Using the notation we defined earlier, the plot can be defined as plotting $j$ on the x-axis and the five point summary of the following groups on the y-axis:
...
The number of interpolation points is much managable than the searching points and always shows a pattern of gradual increase in the index value. Hence a line plot with interpolating order on the x-axis will be desirable. Using notation, the x-axis is a sequence $$x = \{1, 2, \cdots, k_1, \cdots, \sum_{i=1}^Jk_i\}$$ and y-axis are the index values defined by $y = I_{x}$. 
