...
Several R implementations address optimization problems with both general purpose as well as tasks specifics solvers. The most prominent one within the general solvers is *optim* in (REF-stats package) which have functions for gradient based and derivative free methods. Other packages having similar functionalities include *nloptr* (REF), XX. 
...
```{r toy-search, fig.cap="\\label{fig:toy-search} A comparison of search\\_better with different values of the parameter max\\_tries. A six-variable dataset boa6 is used with holes index on a 2D problem. max\\_tries = 500 allows the optimiser to find better basis with higher index value at iteration six."}
```
...
The second experiment with the noisy index is to understand how the optimisers perform when a local maximum is present. The dataset used is `boa6` where `x2` and `x7` are informative. The two theoretical best bases are [`r matrix(c(0, 1, 0, 0, 0, 0), nrow = 6)`] and [`r matrix(c(0, 0, 1, 0, 0, 0), nrow = 6)`] with index value `r index <- tourr::norm_kol(nrow(boa6)); proj <- as.matrix(boa6) %*% matrix(c(0, 1, 0, 0, 0, 0), nrow = 6); index(proj)` and `r index <- tourr::norm_kol(nrow(boa6)); proj <- as.matrix(boa6) %*% matrix(c(0, 0, 1, 0, 0, 0), nrow = 6); index(proj)`, respectively. Hence, the global maximum happens when variable `x7` is found. Simulation is done with 20 randomly generated seeds on two optimisers: `search_better` (Figure \@ref(fig:kol-better)) and `search_better_random` (Figure \@ref(fig:kol-random) and Figure \@ref(fig:kol-random-tuned)). Compared to Figure \@ref(fig:kol-better) and \@ref(fig:kol-random), Figure \@ref(fig:kol-random-tuned) further increases the neighbourhood parameter `alpha` from 0.5 to 0.7 to enlarge the search space.  The data object is collected for each simulation and computation is done to reduce the bases from its original 6D to a 2D points by principal component analysis so as to view the search paths.
