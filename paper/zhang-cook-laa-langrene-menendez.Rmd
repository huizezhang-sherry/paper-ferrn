...
Random variables with different distributions have been simulated, and the distributional form of each variable is presented in equations \ref{eq:sim-norm} to \ref{eq:sim-x7}. Variables `x1`, `x8` to `x10` are normal noise with zero mean, and unit variance and `x2` to `x7` are normal mixtures with varied weights and locations. All the variables have been scaled to have overall unit variance before projection pursuit. The holes index [@cook2008grand], used for detecting bimodality of the variables, is used throughout the examples unless otherwise specified.
