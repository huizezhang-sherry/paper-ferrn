...
Random search [in practice, non-convex, non-smooth]. The original (pure) random search, Brooks, randomly evaluate points in the space regardless of the current position. Although gaurantee global convergence, as the dimensional increases, blindly sample points in the entire space would cost a long time for convergence to occur [spall, 6]. Various algorithms improve on the pure random search by either concentrating on a narrower sampling space or a different updating machanism. Creeping random search, [R.C White], is such a variation where a candidate point is generated within a neighbourhood of the current point and the current point is only updated if the candidate point makes an improvment on the function value. [no convergence guarantee]. A different random search algorithm, simulated annealing, takes a different updating procedure than creeping random search. Rather than only updating the current point when an improvement is made, simulated annealing uses a Metropolis acceptance criterion, where nonimproving candidates can still have a chance to be accepted. The convergence of simulated annealing algorithms has been widely researched [reference, reference, reference] and a global convergence is guaranteed [under what condition].
