...
An animation is another type of display to show how the search progresses from start to finish in the space. Figure `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\ref{fig:toy-pca-animated}','\\@ref(fig:toy-pca-animated-interactive)'))` shows the animated version (six frames from the animation if viewed in pdf) of the PCA plot in Figure `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\ref{fig:toy-pca}','\\@ref(fig:toy-pca)'))`. An additional piece of information one can learn from this animation is that CRS finds its end basis quicker than PD since CRS finishes its search in the 5th frame while PD is still making more progress.
...
```{r toy-pca-animated-interactive, out.extra="class = 'gif'", fig.cap = "Animated version of Figure 4. With animation, the progression of the search paths from start to finish can better identified and CRS (green) finishes the optimisation quicker than PD (brown).", eval=knitr::is_html_output()}
```
...
```{r toy-tour-interactive, out.extra="class = 'tour-gif'",fig.cap="Animating the two paths from Figure 4 and 5 in the full basis space. The full basis space in this example is a 5D unit sphere, on which points (grey) are randomly generated via the CRAN package geozoo.", eval = knitr::is_html_output()}
```
...
Both optimisers have found the third variable in the first direction, and the second variable in the second direction. Note however the different orientation of the basis, following from the different sign in the second column. One would expect to see this in the torus plot as the final bases match each other when projected onto one torus circle (due to the same sign in the first column) and are symmetric when projected onto the other (due the sign difference in the second column). In Figure `r knitr::asis_output(ifelse(knitr::is_latex_output(), '\\ref{fig:toy-torus}','\\@ref(fig:toy-torus-interactive)'))`, this can be seen most clearly in frame 5 where the two circles are rotated into a line from our view.
