...
Animation is another display to show how the search porgresses from start to finish in the space and an  `animate = TRUE` argument is used to enable the animation. Figure \ref{fig:toy-pca-animated} shows six frames from the animation of the PCA plot in Figure \ref{fig:toy-pca}. An additional piece of information one can learn from this animation is that the SA search finds its end basis quicker than the PD search since SA has finished in the 5th frame while PD is still making more progression.
