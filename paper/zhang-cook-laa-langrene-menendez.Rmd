...
A large class of methods utilises the gradient information of the objective function, with the most notable one being the gradient ascent (descent) method. Although gradient optimization methods are popular, they rely on the availability of the objective function derivatives and on the complexity of the constraints. As will be shown in the next section, the independent variable, in our optimisation problem, is a matrix of projection basis and the computational time required to perform differentation on a matrix could impede the render of tour animation. Hence, gradient-based methods are not the focus of this paper and considerations will be given to derivative-free methods. 

Derivative-free methods, which do not rely on the knowledge of the gradient, are more generally applicable. Derivative-free methods have been developed over the years, where the emphasis is on finding, in most cases, a near optimal solution. Below will introduce three derivative-free methods, two of which are random search methods: creeping random search and simulated annealing, and the other one is pseudo derivative search.  

Random search mehtods [@zabinsky2013stochastic] contain a random sampling component in its algorithm and has shown its ability to optimise non-convex and non-smooth functions. The initial random search algorithm, pure random search [@Brooks1958-zk], draws candidate points from the entire space without using any information of the current position and updates the current position when an improvement on the objective function is made. As the dimension of the space becomes larger, sampling from the entire space would indicate a long time for convergence to occur despite a guaranteed global convergence [@spall2005introduction]. Various algorithms have then been developed to improve pure random search by either concentrating on a narrower sampling space or using a different updating machanism. Creeping random search [@White1971-ci] is such a variation where a candidate point is generated within a neighbourhood of the current point. This gives creeping random search an advantage of being fast to compute, however, global convergence is not longer guaranteed. On the other hand, simulated annealing [@kirkpatrick1983optimization; @bertsimas1993simulated], takes a different updating machanism from the pure random search. Rather than only updating the current point when an improvement is made, simulated annealing uses a Metropolis acceptance criterion, where nonimproving candidates can still have a chance to be accepted. The convergence of simulated annealing algorithms has been widely researched [@mitra1986convergence; @belisle1992convergence; @granville1994simulated] and the global optimum can be attained under mild conditions.

Pseudo derivative search uses a common search scheme in optimistaion: line search. In line search methods, users are required to provide an initial estimate $x_{1}$ and, at each iteration, a search direction $S_k$ and a step size $\alpha_k$ are generated. Then one moves on to the next point following $x_{k+1} = x_k + \alpha_kS_k$ and the process is repeated until the desire convergence is reached. In derivative-free methods, local information of the objective function are used to determine the search direction. The choice of step size also needs considerations, as inadequate step sizes might prevent the optimisation method to converge to an optimum. An ideal step size can be chosen via finding the value of $\alpha_k \in \mathbb{R}$ that maximises $f(x_k + \alpha_kS_k)$ with respect to $\alpha_k$ at each iteration. 
