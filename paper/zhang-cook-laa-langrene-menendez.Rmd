...
While the previous few examples have looked at the space of 1D bases in a unit sphere, this section visualises the space of 2D bases. Recall that the columns in a 2D basis are orthogonal to each other, so the space of $p \times 2$ bases is a $p$-D torus. For $p = 3$ one would see a classical 3D torus shape as shown by the grey points in Figure \ref{fig:toy-torus}. The two circles of the torus can be observed to be perpendicular to each other and this can be linked back to the orthogonality. Two paths from CRS and PD are plotted on top of the torus and coloured in green and brown respectively to match the previous plots. The final basis found by PD and CRS are shown in a larger shape  and printed below respectively:
...
Four diagnostic plots have been introduced to investigate the progression of the optimiser and the coverage in search space. The implementation of these visualisations is designed to be easy-to-use, each plot can be produced by supplying the data object to the corresponding function. More advanced users may decide to modify these basic graphs using the provided wrapper functions, or even build their own. 
