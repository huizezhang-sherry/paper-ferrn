...
\noindent where $f$, $\mathcal{A}$, and $\mathbf{X}$ are defined as in the previous section,  $I_d$ is the $d$-dimensional identity matrix and the constraint ensures the projection bases $\mathbf{A}$ to be orthonormal. It is worth noticing the following: 1) The optimisation is constrained and the basis space is a $d$-dimensional sphere. 2) There may be index functions for which the objective function might not be differentiable. 3) While finding the global optimum is the goal of the optimisation problem, interesting projections may also appear in the local optimum. 4) The optimisation should be computationally agile since the tour animation is viewed by the users during the optimisation.
...
```{r toy-pca-animated, fig.cap = "Six frames selected from the animation of search paths in the PCA-projected basis space. With animation, the start and end position of each optimisation routine is clearly identified. The full video of the animation can be found at \\url{https://vimeo.com/504242845}."}
```
...
```{r toy-tour, fig.cap="Six frames selected from the tour view in the original 5D space for the same 1D projection problem as Figure \\ref{fig:toy-pca} and \\ref{fig:toy-pca-animated}. The full video of the animation can be found at \\url{https://vimeo.com/512885379}."}
```
...
```{r toy-torus, fig.cap="Six frames selected from the torus view of 2D basis space. The grey points are randomly generated 2D basis and the two paths are optimised by PD (brown) and SA (green) for a three-variable 2D projection problem using the holes index. The grey points show the space of the 2D basis as a torus. The full video of the animation can be found at \\url{https://vimeo.com/512882784}."}
```
