...
abstract: |
  Guided tour [@cook1995grand] searches for interesting low-dimensional views of high-dimensional data via optimising a projection pursuit index function. The first paper of projection pursuit by @friedman1974projection stated that "the technique used for maximising the projection index strongly influences both the statistical and the computational aspects of the procedure." While much work has been done in proposing  indices in the literature, less has been done on evaluating the performance of the optimisers. In this paper, we propose a data collection object in the optimisation of projection pursuit guided tour and introduce visual diagnostics for examining the performance of the optimisation procedure. These diagnostics and this workflow can be applied to a broad class of optimisers to assess their performance. An R package, \CRANpkg{ferrn} [@ferrn], has been created to implement the diagnostics. 
...
This project contributes to the software development in two packages: a data collection object is implemented in \CRANpkg{tourr} [@tourr], while the visual diagnostics of the optimisers is implemented in \CRANpkg{ferrn} [@ferrn].  Below list functions in the \CRANpkg{ferrn} [@ferrn] package: 
