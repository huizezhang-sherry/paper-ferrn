...
\textcolor{red}{We change the wording of the sentence to "**a torus in the $p$-D space**"}
...
\textcolor{red}{Changed to "We use PCA to estimate the 2D space that contains all the bases recorded and the zero basis. The centre of the 2D view is the first two PC coordinates of the zero matrix, and the radius is estimated as the largest distance from the centre to the basis."}
...
\textcolor{red}{The explanation of data set is introduced in the subsection *Simulation setup* under *Diagnosing an optimiser*. To avoid ambiguity, we change the "original 5D space" to "original space" and give more details about the original space in the figure caption: "The basis space in this example is a 5D unit sphere ...".}
...
\textcolor{red}{A paragraph summarising the finding is added as the second paragram in the conclusion}
...
\textcolor{red}{Added "The black star represents the theoretical best basis the optimisers aiming to find." in the caption of figure 4.}
...
\textcolor{red}{The Kolmogorov Smirnoff index here looks at the difference between a theoretical pdf and an empirical one based on data. I am not sure what would be the reasoning for using normal quantiles in this index. That would imply you are assuming normality on the theoretial pdf.}
