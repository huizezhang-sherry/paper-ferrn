...
**ANS: With p-D bases, the correction of orientation is a bit more complicated. Take 2D as an example, two bases can be aligned in the first axis but not in the second. This would means there are $2^p$ possibility to check. **
...
Probably here the reviewer would like to see a kind of "recipie" about how to use the framework of the paper for other optimisers?
...
**Yes the reviewer is giving a decent explanation in describing the effect of flipping. The gray dot should land on the dashed line and this is a mistake due to the data not being updated after flipping in the code. This has now been corrected. **
...
- We change the wording of the sentence to "**a torus in the $p$-D space**"
...
**The explanation of data set is introduced in the subsection *Simulation setup* under *Diagnosing an optimiser*. To avoid ambiguity, we change the "original 5D space" to "original space" and give more details about the original space in the figure caption: "The basis space in this example is a 5D unit sphere ...". **

It might be worth to mentioned about the 5D in the simulation sep up subsection so that it is introduced explicitly for the readers.
...
And the frames are orthonormal, right?
...
I think the reviewer here wants to see a bit more detail on the conclusions and have provided a number of questions that could be answered in order to add some interesting points on that section.
...
"The black star **represents** the theoretical ...
...
**The Kolmogorov Smirnoff index here looks at the difference between a theoretical pdf and an empirical one based on data. I am not sure what would be the reasoning for using normal quantiles in this index. That would imply you are assuming normality on the theoretial pdf.**
